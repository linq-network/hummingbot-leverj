...
def sign_order(order, instrument_id, decimals, secret):
    (abi_types, evm_parameters) = _get_evm_parameters(
        order, instrument_id, decimals)
    # solidity pack into bytes
    keccak_hash = _get_hash(abi_types, evm_parameters)
    signature = sign(keccak_hash, secret)
    return signature


def _get_evm_parameters(order, instrument_id, decimals):
    abi_types = []
    evm_parameters = []
    ...
    evm_parameters.append(to_int(int(order['timestamp'])))
    # orderType
...
def sign(hash, secret):
    signed_message = w3.eth.account.signHash(hash, secret)
